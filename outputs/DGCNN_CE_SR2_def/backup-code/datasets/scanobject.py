...
def split_dictionary(selected_key, negate = False):
    selected_value = sonn_all[selected_key]
    selected_dict = {k: v for k, v in sonn_all.items() if v == selected_value}
    remaining_dict = {k: v for k, v in sonn_all.items() if k != selected_key and v != selected_value}
    if negate:
...
class ScanObject(data.Dataset):
    def __init__(
        ...
        if self.class_choice is not None:
            ...
            if isinstance(self.class_choice, int):
                ### we're evaluating by class
                if negate: ###we're building dictionary with ALL classes except the selected one
                  self.class_choice = split_dictionary(self.class_choice, negate=True)
                else:
                  self.class_choice = split_dictionary(self.class_choice)#{self.class_choice: sonn_all[self.class_choice]}                
