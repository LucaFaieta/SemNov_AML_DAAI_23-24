...
class PointPatchTransformer(nn.Module):
    ...
    def forward(self, xyz: torch.Tensor):
        ...
        centroids = self.sa(xyz)  # Only sample centroids without features

        x = self.lift(centroids)
...
class Projected(nn.Module):
    ...
    def forward(self, xyz: torch.Tensor, features= torch.full((64, 1024, 6), 0.4).to("cuda"), device=None, quantization_size=0.05):
        return self.proj(self.ppat(
...
def PointBERT():
        return Projected(
            ...
            nn.Linear(256, 1280)) #1024 input of the classification head
